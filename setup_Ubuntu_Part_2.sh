...
function install_hlf ()
    {
        if [[ $HLF_INSTALL == "true" ]]; then
            ...
            echo ' ' >> ~/.bashrc
            echo 'FABRIC_VERSION="hlfv12"' >> ~/.bashrc
...
do
    ...
    if [ -d "${HOME}/.hfc-key-store" ]; then
        cp -Rv $HLF_INSTALL_PATH/fabric-scripts/hlfv12/composer/creds/* ~/.hfc-key-store
    else
        mkdir ~/.hfc-key-store
        cp -Rv $HLF_INSTALL_PATH/fabric-scripts/hlfv12/composer/creds/* ~/.hfc-key-store
    fi
